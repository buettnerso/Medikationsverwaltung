...
- `Medikationsverwaltung_AppIcon.ico` wird über `AppIcon.rc` direkt in die ausführbare Datei eingebettet.
- Das Fenster erhält das Icon zusätzlich über `WM_SETICON` für große und kleine Darstellung.
...
### Behoben
- Datumswerte werden beim Schreiben an Excel als echter COM-Datentyp `VT_DATE` über `Range.Value` übergeben.
...
- Es gibt zwei getrennte Excel-Berichtstypen:
  - `DrugAccount_Gesamt.xlsx` – alle Datensätze eines IMP
  - `DrugAccount_Patient.xlsx` – ein IMP und ein ausgewählter Patient
...
### Vorlage
- `Templates/Bericht_DrugAccount.xlsx` wurde auf das damalige Patientenberichtsschema aktualisiert.
...
| **v8.8** | Responsive Tabellenbreite |
| **v8.7** | `VT_DATE`, fixe Tabellenköpfe |
